...
const setFloatValue = function (data) {
  ...
  if (data?.success && $item[0]) {
    ...
    if (data.iteminfo.paintseed) {
      const gemInfo = SIH?.gemsDetector.GetItemGemByPainSeed($item[0].rgItem.market_hash_name, data.iteminfo.paintseed);

      if (gemInfo) {
        floatDiv.find('.itemseed .value').text(`${data.iteminfo.paintseed} (${gemInfo.gemName} ${gemInfo.tierName})`);
      }
    }
  }
};
  ...
  if ($container.parent().find(`.sih-tooltip__row`).length === 0) {
    const rowEl = `<div class="sih-tooltip__row">
        ...
        <div class="sih-tooltip__right-block sih-tooltip__right-block_keychain">
          <span class="sih-tooltip__price">${price}</span>
          ${pattern ? `<span class="sih-tooltip__percent">Paint seed: ${pattern}</span>` : ''}
        </div>
      </div>`;
...
var prebuildStickersData = function (floatDataArr) {
  $J(floatDataArr).each(async (index, data) => {
    ...
    for (let i = 0; i < imagesDiv.length; i++) {
      ...
      const marketHashName = sticker.name;
    ...
    chrome.runtime.sendMessage(
      SIHID,
      {
        ...
        data: {
          ...
          source: 'listing sticker prices 1',
        },
      },
      (res) => {
        if (res.success) {
    ...
    chrome.runtime.sendMessage(
      SIHID,
      {
        ...
        data: {
          ...
          markets: ['steamLiveSell', 'buff163'],
          items: marketHashNameArr,
          source: 'listing sticker prices 2',
        },
      },
      (res) => {
        if (res.success) {
...
function loadRate() {
  ExchangeRates.GetRate((isLoad) => {
    if (isLoad) {
      SIH?.WaitLargeItemInfoRendering().then(() => {
...
if (IS_ENABLED_SIH) {
  ...
  $J.ajax = function (req) {
    if (req.url.includes('/market/itemordershistogram')) {
      SIH?.MarketCommodityStore?.setItemNameId(req.data.item_nameid);
      req.data.currency = SIH?.marketCommodityOrderBlock?.getCurrency() || req.data.currency;
    }

    return AjaxMaster(req)
      .success((res) => {
        if (req.url.includes('/market/itemordershistogram') && res?.success === 1) {
          ...
          try {
            ...
            console.error('sendItemData error', e);
          }
