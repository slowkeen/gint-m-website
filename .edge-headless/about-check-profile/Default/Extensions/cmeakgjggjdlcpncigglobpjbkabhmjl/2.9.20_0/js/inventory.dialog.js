function loadDialogHTML() {
  let dialog = `<div id="group_inventory_dialog" class="newmodal" style="display: none; min-width: 600px">
...
const GroupInventoryDialog = {
  ...
  OnReload: function (event) {
    const _this = this;
    event.stop();
    const imageName = this.m_rgItem.description.icon_url_large
      ? this.m_rgItem.description.icon_url_large
      : this.m_rgItem.description.icon_url;
    const url = ImageURL(imageName, 330, 192);
    $J('#group_inventory_dialog_icon').attr('src', url);

    this.m_rgItem.groupInventories.sort((a, b) =>
    ...
    if (this.m_isWeapon) {
      ...
      const getCountSelectedGroupInventory = () =>
        this.m_rgItem.groupInventories.filter((x) => x.isSelectedForSell).length;
      ...
      this.m_rgItem.groupInventories.forEach((item, ind) => {
        ...
        const actionLink = SIH?.global?.PrepareInspectLink(actions, asset_properties);
        ...
        $elem.find(`.${uiInd}_row`).click(function () {
          ...
          $uniqueInventory
            .find('.selected_sell_btn')
            .text(SIHLang.selectItemCount.replace('$1', getCountSelectedGroupInventory()));
        });

        const stickersHtml = (item.description.descriptions.find((x) => x.value.indexOf('sticker_info') > -1) || {})
          .value;
        if (stickersHtml) {
      ...
      $elem.find('.sih-table-row.head .col_sort').click(function () {
        ...
        arr.sort(function (a, b) {
          ...
          if ((typeSort === 'asc' && aValue > bValue) || (typeSort === 'desc' && aValue < bValue)) {
            return 1;
          } else if ((typeSort === 'asc' && aValue < bValue) || (typeSort === 'desc' && aValue > bValue)) {
            return -1;
          } else {
            return 0;
          }
    ...
    $J('#group_inventory_dialog_content')
      .find('.inventory-total-trade-ban')
      .html(this.m_rgItem.description.useCountMarketHashName);

    $J('#group_inventory_dialog_content').find('.inventory-price').text('---');
    SIH?.itemDescription?.LoadPriceSteamInventory(this.m_rgItem.description.market_hash_name).done((data) => {
