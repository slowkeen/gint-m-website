  ...
  chrome.storage.local.get(
    ...
    function (items) {
      var strRes =
        ...
        items.totalplus +
        ' - Profit: ' +
        Math.round((items.totalminus - items.totalplus) * 100) / 100;
...
var timer = null;
var regHis =
  /<div class="market_listing_left_cell market_listing_gainorloss">\s+?([\+\-])\s+?<\/div>[\s\S]+?<span class="market_listing_price">([\s\S]+?)<\/span>[\s\S]+?<div class="market_listing_right_cell market_listing_listed_date can_combine">([\s\S]+?)<\/div>[\s\S]+?<div class="market_listing_right_cell market_listing_listed_date can_combine">([\s\S]+?)<\/div>/gim;
...
var ShowTable = function () {
  chrome.storage.local.get(
    ...
    function (items) {
      ...
      var ngraph = items.graphdata;
      strRes +=
        '<table border="1" class="table"><thead><tr><th>Date</th><th colspan="2">Sold</th><th>Acc. sold</th><th colspan="2">Bought</th><th>Acc. bought</th><th>Profit</th><th>Acc. profit</th></tr></thead><tbody>';
      var accm = 0,
        accp = 0;
      ...
      for (var i = 0; i < ngraph.length; i++) {
        ...
        strRes +=
          ...
          accp / 100 +
          '</td><td>' +
          ((ngraph[i].totalm - ngraph[i].totalp) / 100).toFixed(2) +
          '</td><td>' +
          ((accm - accp) / 100).toFixed(2) +
          '</td></tr>';
      }
      strRes += '</tbody></table>';
      $('#spanGraphData').html(strRes);
...
var ProcessPriceData = function (res) {
  ...
  chrome.storage.local.get(
    ...
    function (items) {
      ...
      var nTotalM = Math.round((items.totalminus + totalMinus) * 100) / 100,
        nTotalP = Math.round((items.totalplus + totalPlus) * 100) / 100;
