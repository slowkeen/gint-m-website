      ...
      onSuccess: function (transport) {
        ...
        if (transport.responseJSON && transport.responseJSON.success) {
          ...
          if (result == 1 || result == 22) {
            $J('[data-giftid=' + gidGift + '] .loading').text('Done');
      ...
      if (res && res.success) {
        ...
        if (giftsID.length) {
          ...
          for (var i = 0; i < giftsID.length; i++) {
            ...
            var elGift =
              '<div class="sendgift_review_item checkout_review_cart_item even" data-giftid="' +
              l_gift.id +
              '">' +
              '<div class="checkout_review_item_img">' +
              '<img src="//steamcommunity-a.akamaihd.net/economy/image/' +
              l_gift.des.icon_url_large +
              '/120x45" width="120" height="45" border="0">' +
              '</div>' +
