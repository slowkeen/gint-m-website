...
const { COUNTRY = 'US' } = $J('#application_config').data('config');
...
BuyCardSetDialog = {
  ...
  ProcessBuyQueue: function () {
    if (this.m_oBuyQueue.length) {
      ...
      $row.find('.sih-card-price').css({
        background: `url(${window.location.protocol}//steamcommunity-a.akamaihd.net/public/images/login/throbber.gif) no-repeat`,
      });
      $J.ajax({
        ...
        .done(function (response) {
          if (response.success == 29) {
            ...
            $row.find('.market_listing_price.market_listing_price_with_fee').text('Error');
  ...
  BuyFirstCard: function () {
    ...
    cardsToBuy.each(function () {
      ...
      var data = {
        ...
        currency: g_rgWalletInfo ? g_rgWalletInfo.wallet_currency : steamCurrency.eCurrencyCode,
        appid: rdata.appid,
  ...
  OnReload: function (event) {
    ...
    $J('.badge_card_to_collect').each(function (i, e) {
      ...
      var priceLink =
        ...
        COUNTRY +
        '&currency=' +
        steamCurrency.eCurrencyCode +
        '&market_hash_name=' +
        marketHashname;
      var row = $J(`<div class="market_listing_row market_recent_listing_row" data-name="${marketHashname}">`);
      row.append(
        '<div class="market_listing_item_img_container"><img src="' +
          imgUrl +
          '" class="market_listing_item_img" /></div>'
      );
      row.append(
      ...
      row.append(
        '<div class="market_listing_item_name_block">' +
          ...
          cardName +
          '</span><br /><span class="market_listing_game_name">' +
          appID +
          ' ' +
          marketHashname.replace(/%23/g, '#') +
          '</span>' +
    ...
    $J('.badge_card_set_card.owned').each(function (i, e) {
      ...
      row.append(
        '<div class="market_listing_item_name_block">' +
          ...
          '<span class="market_listing_item_name">' +
          badgeSeries +
          '</span><br /><span class="market_listing_game_name">753 ' +
          marketHashname.replace(/%23/g, '#') +
          '</span>' +
          '</div><div style="clear: both"></div>'
      );
        ...
        .catch(() => {
          ...
          if (altName.indexOf('(Trading Card)') == -1) {
            ...
            RequestCacher.get({
              method: 'GET',
              url:
                '//steamcommunity.com/market/priceoverview/?appid=753&country=' +
                COUNTRY +
                '&currency=' +
                steamCurrency.eCurrencyCode +
                '&market_hash_name=' +
  ...
  CalculateTotal: function () {
    ...
    $J('#market_buycardset_dialog_buyall span').text(
      SIHLang.tradingcards.buyall + ' (' + BuyCardSetDialog.m_total.toFixed(2) + ')'
    );
  },
...
var AddBuyCardSet = function () {
  var dialog =
    '<div id="market_buycardset_dialog" class="newmodal" style="display: none; min-width: 570px">' +
    '<div class="newmodal_header_border">' +
    '<div class="newmodal_header">' +
    '<span id="market_sell_dialog_title">' +
    SIHLang.tradingcards.dialogtitle +
    '</span>' +
    ...
    '<a id="market_buycardset_dialog_buyall" href="#" class="btn_green_white_innerfade btn_small_wide" style="margin-right:10px"><span>' +
    SIHLang.tradingcards.buyall +
    '</span></a>' +
  ...
  dialog +=
    '<div id="hover" style="display: none; z-index: 1000">' +
...
function GetPriceValueAsInt(strAmount) {
  ...
  strAmount = strAmount
    .replace(
      GetCurrencySymbol(
        GetCurrencyCode(g_rgWalletInfo ? g_rgWalletInfo['wallet_currency'] : steamCurrency.eCurrencyCode)
      ),
      ''
    )
    .replace(',', '.')
