...
function checkAbuseSub(elForm) {
  ...
  $J.post('https://steamcommunity.com/actions/ReportAbuse/', params)
    .done(function () {
      ShowAlertDialog(
        'Thank You!',
        'Thank you for reporting offensive content and helping to keep the Steam Community clean and friendly.'
      );
    })
    .fail(function () {
...
function AlertNonSteamSite(elem) {
  ...
  if (hostname) {
    ...
    for (var i = 0; i < g_whiteListedDomains.length; ++i) {
      ...
      if (
        index != -1 &&
        index == hostname.length - g_whiteListedDomains[i].length &&
...
function FilterListFast(target, str) {
  ...
  str = str.toLowerCase();
  if (str == lastFilter) return false;

  var expanding = false;
  var contracting = false;
  if (str.length > lastFilter.length && str.startsWith(lastFilter)) expanding = true;
  if (!str || (str.length < lastFilter.length && lastFilter.startsWith(str))) contracting = true;
  ...
  for (var i = 0; i < rgChildren.length; i++) {
    ...
    if ((expanding && child.style.display == 'none') || (contracting && child.style.display != 'none')) continue;
    if (!child.lcText) child.lcText = (child.innerText || child.textContent).toLowerCase();

    var text = child.lcText;
    var show = true;
    for (var iPart = 0; show && iPart < strParts.length; iPart++) if (!text.include(strParts[iPart])) show = false;
...
function v_currencyformat(valueInCents, currencyCode, countryCode) {
  ...
  if (g_rgCurrencyData[currencyCode]) {
    ...
    const decimal_formatted = `${currencyData.strDecimalSymbol}${Math.abs(valueInCents - currencyFormat)
      .toFixed(2)
      .slice(2)}`;
...
function GetCurrencyCode(currencyId) {
  for (var code in g_rgCurrencyData) {
    if (g_rgCurrencyData[code].eCurrencyCode == currencyId) return code;
  }
  return 'Unknown';
}
...
function GetCurrencySymbolFromString(stringWithSymbol) {
  const re =
    /(?:AED|ARS\$|A\$|R\$|Br|CDN\$|CHF|CLP\$|¥|COL\$|₡|€|£|HK\$|Rp|₪|₹|¥|₩|KD|₸|Mex\$|RM|kr|원|NZ\$|S\/\.|P|zł|QR|刀币|руб\.|SR|S\$|฿|TL|NT\$|₴|\$|\$U|₫|R)/;
...
function formatNumberV2(totalPrice) {
  let currencyID = GetCookie('steamCurrencyId');
  if (!currencyID) {
    currencyID = 1;
  }
  const currencyCode = +(typeof g_rgWalletInfo != 'undefined' && g_rgWalletInfo['wallet_currency']
...
function formatNumber(totalPrice) {
  if (
    v_currencyformat &&
    GetCurrencyCode &&
    (typeof currencyId != 'undefined' || typeof g_rgWalletInfo != 'undefined')
  ) {
...
function GetAvatarURLFromHash(hash, size) {
  var strURL =
    'https://cdn.akamai.steamstatic.com/steamcommunity/public/images/avatars/' + hash.substring(0, 2) + '/' + hash;

  if (size == 'full') strURL += '_full.jpg';
  else if (size == 'medium') strURL += '_medium.jpg';
  else strURL += '.jpg';
...
function ShowAbuseDialog() {
  if (!g_AbuseModalContents) g_AbuseModalContents = $J('#reportAbuseModalContents');
...
function StandardCommunityBan(steamid, elemLink) {
  $J.get('https://steamcommunity.com/actions/communitybandialog', { sessionID: g_sessionID, steamID: steamid })
    .done(function (data) {
      var $Content = $J(data);
      var Modal = ShowConfirmDialog('Community Ban & Content Removal', $Content, 'Submit').done(function () {
    ...
    .fail(function (data) {
      ShowAlertDialog(
        'Community Ban & Delete Comments',
        'You do not have permissions to view this or you are not logged in.'
      );
    });
}
...
function CEmoticonPopup($EmoticonButton, $Textarea) {
  ...
  if (CEmoticonPopup.sm_deferEmoticonsLoaded == null) CEmoticonPopup.sm_deferEmoticonsLoaded = new jQuery.Deferred();
...
CEmoticonPopup.prototype.OnButtonClick = function () {
  if (this.m_bVisible) {
    ...
    if (!this.m_$Popup) this.BuildPopup();
    else PositionEmoticonHover(this.m_$Popup, this.m_$EmoticonButton);
    ...
    if (window.UseSmallScreenMode && window.UseSmallScreenMode()) {
      ...
      $J(window).scrollTop(
        this.m_$EmoticonButton.offset().top -
          $J(window).height() +
          this.m_$Popup.height() +
          this.m_$EmoticonButton.height() +
          10
      );
    }
...
CEmoticonPopup.prototype.GetEmoticonClickClosure = function (strEmoticonName) {
  ...
  return function () {
    ...
    if (elTextArea) {
      ...
      elTextArea.value =
        elTextArea.value.substr(0, nSelectionStart) + strTextToInsert + elTextArea.value.substr(nSelectionStart);
    ...
    if (window.DismissEmoticonHover) window.setTimeout(DismissEmoticonHover, 1);
  };
};
...
function PositionEmoticonHover($Hover, $Target) {
  ...
  if ($Target.height() < 48) nTopAdjustment = Math.floor($Target.height() / 2) - 12;
    ...
    if (rgItemKey.length >= 3 && rgItemKey.length <= 5) {
      ...
      if (appid == 'classinfo') {
        ...
        if (rgItemKey.length == 4 && rgItemKey[3]) {
          ...
          if (strOwner.indexOf('id:') == 0) strURL += '&o_url=' + strOwner.substr(3);
          else strURL += '&o=' + strOwner;
        }
      }
      if (nAmount && nAmount > 1) strURL += '&amount=' + nAmount;
...
function ChangeLanguage(strTargetLanguage, bStayOnPage) {
  ...
  $J.post('https://steamcommunity.com/actions/SetLanguage/', { language: strTargetLanguage, sessionid: g_sessionID })
    .done(function () {
      if (bStayOnPage) Modal.Dismiss();
      else {
        if (g_steamID) window.location = 'https://store.steampowered.com/account/languagepreferences/';
...
function UpdateParameterInCurrentURL(strParamName, strParamValue, rgRemoveParameters) {
  ...
  var params = {};
  if (query && query.length > 2) params = $J.deparam(query.substr(1));

  if (strParamValue === null) delete params[strParamName];
  else params[strParamName] = strParamValue;

  // comment thread specific
  if (rgRemoveParameters) for (var i = 0; i < rgRemoveParameters.length; i++) delete params[rgRemoveParameters[i]];
...
function InitializeCommentThread(type, name, rgCommentData, url, nQuoteBoxHeight) {
  ...
  if (window['CCommentThread' + type]) commentclass = window['CCommentThread' + type];
...
function BindCommentThreadSubscribeButtons(type, owner, gidfeature, gidfeature2, btnSubscribe, btnUnsubscribe) {
  ...
  if (CommentThread) CommentThread.BindSubscribeButtons(btnSubscribe, btnUnsubscribe);
}

function FindCommentThread(type, owner, gidFeature, gidFeature2) {
  for (var key in g_rgCommentThreads) {
    if (g_rgCommentThreads[key].BMatches(type, owner, gidFeature, gidFeature2)) return g_rgCommentThreads[key];
  }
  return null;
}
...
function TargetIsChild(event, selector) {
  ...
  if (!reltarget || !$(reltarget).up(selector)) return false;
  return true;
}
...
window.ShowHover = function ShowHover(elem, item) {
  var hover = $('hover');
  ...
  hover.style.top = (boundingRect.top + window.scrollY - 105) / zoom + 'px';
  ...
  var boxRightViewport =
    elem.viewportOffset().left +
    parseInt(elem.getDimensions().width) +
    hover_box.getWidth() +
    (24 - nHoverHorizontalPadding);
  ...
  if (boxRightViewport > document.viewport.getWidth() && nSpaceLeft > nSpaceRight) {
    ...
    hover.style.left =
      parseInt(hover.style.left) + parseInt(elem.getDimensions().width) - nHoverHorizontalPadding + 'px';
  }
  ...
  if (boxTopViewport + hover_box.getHeight() + 8 > document.viewport.getHeight()) {
    ...
    if (hover_arrow) hover_arrow.style.top = '';
  }
